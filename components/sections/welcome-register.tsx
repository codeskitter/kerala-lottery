...
export default function WelcomeRegisterSection({ contentData }: { contentData?: ContentData }) {
  ...
  const supportPhone = contentData?.siteSettings?.support_phone || "+91 96686 43802"
  ...
  const accountName = process.env.NEXT_PUBLIC_ACCOUNT_NAME || "Kerala Jackpot Lottery"
  const accountNumber = process.env.NEXT_PUBLIC_ACCOUNT_NUMBER || "0000000000"
  const ifsc = process.env.NEXT_PUBLIC_IFSC || "SBIN000000"
  ...
  async function handleStep2Submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Submitted successfully. Payment will be verified and we will contact you soon.")
      setStep(1)
      setForm1({
        lottery: "Kerala State Mega Lottery",
        ticketNumber: "",
  ...
  return (
    <section className="bg-background w-full">
      <div className="mx-auto grid w-full grid-cols-1 gap-8 px-4 md:px-8 py-12 md:grid-cols-2 lg:py-12 bg-slate-100 text-left">
        ...
        <div>
          <h2 className="font-heading text-pretty text-3xl font-extrabold md:text-4xl text-black">
            {siteName} — <span className="text-brand">OFFICIAL SITE</span>
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">Welcome to the State Kerala Jackpot Lottery</p>
          <div className="mt-3 flex items-center justify-center gap-2 text-sm text-[#065f46] md:justify-start">
            ...
            <span className="text-2xl font-bold">{supportPhone}</span>
          </div>

          <p className="text-pretty mt-4 max-w-prose text-sm leading-relaxed text-muted-foreground">
            Playing the lottery is your chance to win big and change your life forever. Our online system makes it easy
            to buy tickets securely and check results quickly. Register now and participate in upcoming draws.
          </p>
            ...
            {step === 1 ? (
              ...
              <form onSubmit={handleStep2Submit} className="mt-4 space-y-3">
                ...
                <div className="rounded-md border bg-white p-3">
                  <p className="text-sm">
                    Pay {amount} via UPI to <span className="font-semibold">{upiId}</span>, then enter your
      ...
      {/* Dedicated section for UPI & Bank Details */}
    </section>
  )
}
